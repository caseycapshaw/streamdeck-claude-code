# Claude Code + Apple Terminal — Stream Deck Setup

Stream Deck profile for controlling Claude Code in Apple's built-in Terminal app.

Row 1 buttons open Terminal and launch a Claude Code session. Rows 2–3 send slash commands and hotkeys to an active session.
...
Each button opens Terminal, waits for it to focus, then types a Claude Code launch command.

| Button | Action Type | Config |
|--------|-----------|--------|
| START | Multi Action | Open Terminal.app → Delay 500ms → Text: `claude` + Enter |
| CONTINUE | Multi Action | Open Terminal.app → Delay 500ms → Text: `claude -c` + Enter |
| RESUME | Multi Action | Open Terminal.app → Delay 500ms → Text: `claude --resume` + Enter |
| YOLO | Multi Action | Open Terminal.app → Delay 500ms → Text: `claude --dangerously-skip-permissions` + Enter |
...
1. **Step 1 — Open**: Action → "Open", Path → `/System/Applications/Utilities/Terminal.app`
...
These assume Terminal with an active Claude Code session is already focused.
