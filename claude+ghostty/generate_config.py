#!/usr/bin/env python3
"""Generate Stream Deck profile config for Claude Code buttons."""
...
def make_open_ghostty():
    return {
        ...
        "Resources": None,
        "Settings": {"path": '"/Applications/Ghostty.app"'},
        "State": 0,
        "States": [{"Title": "Ghostty"}],
        "UUID": "com.elgato.streamdeck.system.open",
    }
...
def ghostty_launch_action(command, pos):
    """Multi Action: Open Ghostty → Delay 500ms → Text command + Enter."""
    return make_multi_action(
        [make_open_ghostty(), make_delay(500), make_text_step(command, send_enter=True)],
        img(pos),
    )
...
def build_actions():
    ...
    # 0,0: Start Claude
    actions["0,0"] = ghostty_launch_action("claude", "0,0")
    # 1,0: Continue Session
    actions["1,0"] = ghostty_launch_action("claude -c", "1,0")
    # 2,0: Resume Session
    actions["2,0"] = ghostty_launch_action("claude --resume", "2,0")
    # 3,0: YOLO Mode
    actions["3,0"] = ghostty_launch_action(
